...
export default function PersonalCourseBooking({ userDetail }) {
    ...
    const renderUserRegisteredCourseList = () => {
        return chiTietKhoaHocGhiDanh.map((course, index) => {
            return (
                <div key={index} className='bg-purple-100 grid lg:grid-cols-6 grid-cols-3 text-center shadow-lg lg:h-24 h-40 my-4 border py-4 rounded-xl'>
                    ...
                    <div className="lg:ml-2 mt-3 col-span-3 text-center font-bold lg:text-left">
                        <p> {course.tenKhoaHoc.substring(0, 40)}{course.tenKhoaHoc.length > 40 ? '...' : ''}</p>
