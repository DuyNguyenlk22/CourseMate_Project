...
import EnrollmentUser from "./EnrollmentUser/EnrollmentUser";
export default function UserManagement() {
  const columns = [
    ...
    { title: "Phone Number", dataIndex: "soDt", key: "soDt" },
    {
      ...
      key: "maLoaiNguoiDung",
      render: (text) => {
        if (text === "HV") return <Tag color='green'>Học Viên</Tag>;
  ...
  const handleCancelEroll = () => { setIsModalEnrollOpen(false); };
  ...
  const handleCancelEdit = () => { setIsModalEditOpen(false); };
  ...
  const handleDelete = (taiKhoan) => { dispatch(deleteUser(taiKhoan)); };
  ...
  const handleSearch = (e) => {
    dispatch(searchUser(e.target.value))
      ...
      .catch((error) => {
        message.error("Searching Error:", error);
      });
  };
